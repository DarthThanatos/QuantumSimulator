...
def telegraph_noise(t, sites, qstate, freq, amp):
    nu = t * freq
    mu = cm.sqrt((2.0 * amp * (1 / freq)) ** 2 - 1.0)  # Noise parameters
...
def quantum_discord(t, rho):
    ...
    for theta in range(0, 180, 9):
        for phi in range(0, 360, 12):
            ...
            rho_c1 = von_entropy(
                (tensor(Evec0, qeye(sites)) * rho * tensor(Evec0, qeye(sites))) / p0)  # measurement induced state
...
def qubit_gen(theta, phi):
    psi = cos(radians(theta)) * ket0 + complex(cos(radians(phi)), sin(radians(phi))) * sin(
        radians(theta)) * ket1  # general forma a qubit
    return ket2dm(psi)
